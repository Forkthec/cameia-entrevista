...
* **Native API versioning (Spring Framework 7+):** Since Spring Boot 4.0, prefer the built-in `version` attribute on `@RequestMapping`/`@GetMapping`/`@PostMapping` (etc.) over hand-rolled versioning. Explicitly configure the versioning strategy via `spring.mvc.apiversion.*` (or an `ApiVersionStrategy`/`WebMvcConfigurer` bean) rather than leaving it implicit in the URL convention. Supported strategies are path, header, query parameter, and media type — pick one and document it; it's disabled by default until configured.
